...
class HitokotoSystem:
    ...
    def add_hitokoto(self, content, author=None, source=None, category=None, like=0):
        """添加一条新的一言"""
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    sql = '''
                    INSERT INTO hitokoto (content, author, source, category, `like`)
                    VALUES (%s, %s, %s, %s, %s)
                    '''
                    cursor.execute(sql, (content, author, source, category, like))
                conn.commit()
    ...
    def update_hitokoto(self, hitokoto_id, content=None, author=None, source=None, category=None, like=None):
        """更新一言内容"""
        update_fields = []
        params = []
    ...
    def get_like_count(self, hitokoto_id):
        sql = 'SELECT `like` FROM hitokoto WHERE id = %s'
    ...
    def like_hitokoto(self, hitokoto_id):
        # 使用update_like_count方法更新点赞数
        success = self.update_like_count(hitokoto_id)
        if success:
            # 获取更新后的点赞数
            return self.get_like_count(hitokoto_id)
        else:
            return None

    def update_like_count(self, hitokoto_id, increment=1):
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    sql = '''
                    ...
                    WHERE id = %s
                    '''
                    cursor.execute(sql, (increment, hitokoto_id))
                conn.commit()
                return cursor.rowcount > 0
        except pymysql.Error as e:
            print(f"更新点赞数失败: {e}")
...
if __name__ == "__main__":
    ...
    # )
    # hitokoto.add_hitokoto(
    #     content="世界上只有一种真正的英雄主义，那就是在recognize生活的真相后依然热爱它。",
    #     author="罗曼·罗兰",
    #     source="《约翰·克利斯朵夫》",
    #     category="哲理"
    # )
