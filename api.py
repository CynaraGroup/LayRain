...
def get_hitokoto():
    ...
    if hitokoto_data:
        # 获取点赞数
        like_count = hitokoto.get_like_count(hitokoto_data['id'])
        # 添加点赞数字段
        hitokoto_data['like'] = like_count
        return jsonify({
            'code': 200,
            'message': '获取成功',
            'data': hitokoto_data
...
def get_hitokoto_by_id(hitokoto_id):
    ...
    if hitokoto_data:
        # 获取点赞数
        like_count = hitokoto.get_like_count(hitokoto_id)
        # 添加点赞数字段
        hitokoto_data['like'] = like_count
        return jsonify({
            'code': 200,
...
def list_hitokoto():
    ...
    print(f"接收到的参数: page={page}, pageSize={pageSize}, category={category}, offset={offset}")  # 输出接收到的参数

    hitokoto_list = hitokoto.list_hitokoto(limit=pageSize, offset=offset, category=category)
    total = hitokoto.count_hitokoto(category)

    # 为每个一言添加点赞数字段
    for hitokoto_item in hitokoto_list:
        like_count = hitokoto.get_like_count(hitokoto_item['id'])
        hitokoto_item['like'] = like_count
...
@app.route('/api/hitokoto/<int:hitokoto_id>/like', methods=['POST'])
def like_hitokoto(hitokoto_id):
    """给一言点赞"""
    # 先检查一言是否存在
    hitokoto_data = hitokoto.get_hitokoto_by_id(hitokoto_id)
    ...
    # 点赞并获取新的点赞数
    new_like_count = hitokoto.like_hitokoto(hitokoto_id)
...
@app.route('/api/hitokoto/<int:hitokoto_id>/like/count', methods=['GET'])
def get_like_count(hitokoto_id):
    """获取一言的点赞数"""
    # 先检查一言是否存在
    hitokoto_data = hitokoto.get_hitokoto_by_id(hitokoto_id)
    if not hitokoto_data:
...
if __name__ == '__main__':
    app.run(host="::1",debug=True,port=5000)